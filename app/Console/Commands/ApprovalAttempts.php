...
class ApprovalAttempts extends Command
{
    /**
    ...
    public function handle()
    {
        ...
        foreach($attendees_decode as $a) {
            ...
            $time = Carbon::parse($a['confend'])->toDateTimeString();
            ...
            if($diff >= "48:00" && $end->gt($now))
            {
                ...
                Mail::send('cron_emails.reminder_approve', $data, function ($mail) use ($data) {
                    ...
                    $mail->subject($data['formdata']['confname'].' Reminder: REVIEW NEEDED - Conference Request Submission');
                });  
            }
            else
            {
